...
public class RaftOptionsBuilder {
    ...
    public static RaftOptions initRaftOptions(RaftConfig raftConfig) {
        ...
        raftOptions.setMaxByteCountPerRpc(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.RAFT_MAX_BYTE_COUNT_PER_RPC),
                RaftConstants.DEFAULT_MAX_BYTE_COUNT_PER_RPC));

        raftOptions.setMaxEntriesSize(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.MAX_ENTRIES_SIZE),
                        RaftConstants.DEFAULT_MAX_ENTRIES_SIZE));

        raftOptions.setMaxBodySize(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.MAX_BODY_SIZE),
                        RaftConstants.DEFAULT_MAX_BODY_SIZE)
        );

        raftOptions.setMaxAppendBufferSize(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.MAX_APPEND_BUFFER_SIZE),
                        RaftConstants.DEFAULT_MAX_APPEND_BUFFER_SIZE));

        raftOptions.setMaxElectionDelayMs(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.MAX_ELECTION_DELAY_MS),
                        RaftConstants.DEFAULT_MAX_ELECTION_DELAY_MS)
        );

        raftOptions.setElectionHeartbeatFactor(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.ELECTION_HEARTBEAT_FACTOR),
                        RaftConstants.DEFAULT_ELECTION_HEARTBEAT_FACTOR)
        );

        raftOptions.setApplyBatch(
                ConvertUtils.toInt(raftConfig.getValue(RaftConstants.APPLY_BATCH),
                        RaftConstants.DEFAULT_APPLY_BATCH)
        );

        raftOptions.setSync(
                ConvertUtils.toBoolean(raftConfig.getValue(RaftConstants.SYNC),
